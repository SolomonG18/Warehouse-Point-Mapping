...
def _prepare_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    # Handle files that may or may not have headers.
    # Expect lat in first column, lon in second column.
    df = df.copy()
    if df.shape[1] < 2:
        raise ValueError("CSV must have at least two columns (latitude in column A and longitude in column B).")
    # If the first row contains strings like 'latitude'/'longitude', keep them; else coerce numeric.
...
if use_sample and not uploaded:
    ...
    if uploaded:
        ...
        # Try again with header inference if headerless parse failed
        if df_raw.shape[1] < 2:
            uploaded.seek(0)
...
st.write("Use the color picker to set a unique color for each warehouse point.")

# Defaults: prefill colors if empty
...
color_col = st.column_config.ColorColumn("color", help="Pick a color per warehouse point")
edited = st.data_editor(
    ...
    column_config={
        "color": color_col,
        "latitude": st.column_config.NumberColumn("latitude", help="Decimal latitude", step=0.000001),
        "longitude": st.column_config.NumberColumn("longitude", help="Decimal longitude", step=0.000001),
    },
    key="editor",
)
...
tile_layer = pdk.Layer(
    ...
    pickable=False,
    data_format="",
    # deck.gl will use the 'data' prop for URL template when using TileLayer in pydeck
    # but pydeck does not expose it directly; instead we pass it via 'map_style' workaround by setting map_style=None
)

# In pydeck, to draw raster tiles as a base, we set map_style=None and add a TileLayer that points to 'data' template.
# pydeck's python API doesn't have a 'data' parameter for TileLayer, so we use the raw dict API to ensure the URL template is set.
tile_layer = pdk.Layer.from_json({
    "@@type": "TileLayer",
    "data": tile_url,
    "tileSize": 256,
    "minZoom": 0,
    "maxZoom": 20,
    "renderSubLayers": {
        "@@function": "x => new deck.BitmapLayer(x, x.tile, {desaturate: 0})"
    }
})
...
if edited.shape[0] > 0:
    ...
    center_lat, center_lon, zoom = 39.5, -98.35, 3  # Continental US
...
r = pdk.Deck(
    ...
    map_style=None,  # disable default mapbox basemap
